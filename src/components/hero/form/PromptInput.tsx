  ...
  return (
    <div className="mb-6">
      ...
      <div className="relative">
        <div className="bg-card/30 backdrop-blur-xl p-1 rounded-xl shadow-sm border border-primary/20">
          <Input 
            ref={inputRef}
            type="text" 
            ...
            className="w-full border-0 text-base rounded-xl px-4 py-4 focus:ring-2 focus:ring-primary bg-background/50 shadow-sm transition-all duration-200 placeholder:text-muted-foreground/70"
          />
        </div>
        
        {prompt.length === 0 && (
          <div className="absolute right-6 top-1/2 transform -translate-y-1/2 text-muted-foreground flex items-center text-sm pointer-events-none">
      ...
      <div className="mt-3 flex items-center justify-between text-sm">
        ...
        <span className={`font-medium tabular-nums ${
          prompt.length > 800 ? 'text-amber-500' : 
          prompt.length > 950 ? 'text-red-500' : 'text-primary'
        }`}>
          {prompt.length}/1000
        </span>
      </div>
