...
export const HeroSection = () => {
  ...
  const handleNewGalleryRow = (row: {
    ...
    // Update generation count for conversion tracking
    setGenerationCount(prev => prev + row.length);
  };
  ...
  // Get user activity data for conversion tracking
  const userActivity = {
    generationCount,
    lastImageUrl: generatedImageUrl,
    lastPrompt: galleryImages[galleryImages.length - 1]?.prompt,
    timeSpent: Date.now() - (performance.timing?.navigationStart || Date.now())
  };

  return (
    <ConversionManager pageId="hero_section" userActivity={userActivity}>
